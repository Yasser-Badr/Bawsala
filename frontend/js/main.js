// المسار: frontend/js/main.js

// مصفوفة ألوان هادئة للبطاقات
...
async function fetchLatestArticles() {
    try {
        const response = await fetch('/api/articles?page=1&limit=9');
        const data = await response.json();
        ...
        if (data.data && data.data.length > 0) {
            data.data.forEach(article => {
                ...
                // تطبيق اللون العشوائي كخلفية للبطاقة
                const cardHTML = `
                    <div class="col">
                        <a href="article.html?slug=${article.Slug}" class="text-decoration-none">
                            <div class="card h-100 shadow-sm border-0 rounded-4 overflow-hidden article-card-hover" style="background-color: ${randomColor};">
                                ...
                                <div class="card-body p-4 d-flex flex-column">
                                    <span class="badge bg-white text-primary mb-3 rounded-pill align-self-start fs-6 px-3 py-2 shadow-sm">${categoryName}</span>
            ...
            grid.innerHTML = `<div class="col-12 text-center text-muted py-5"><h4>لا توجد مقالات منشورة بعد.</h4></div>`;
        }
    } catch (error) { console.error(error); }
}
